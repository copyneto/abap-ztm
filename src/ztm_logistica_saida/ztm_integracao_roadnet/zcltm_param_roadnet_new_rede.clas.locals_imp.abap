CLASS lcl_Redespacho DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.

    METHODS read FOR READ
      IMPORTING keys FOR READ _Redespacho RESULT result.

    METHODS rba_Roadnet FOR READ
      IMPORTING keys_rba FOR READ _Redespacho\_Roadnet FULL result_requested RESULT result LINK association_links.

    METHODS importar FOR MODIFY
      IMPORTING keys FOR ACTION _Redespacho~importar_rede.

    METHODS get_features FOR FEATURES
      IMPORTING keys REQUEST requested_features FOR _Redespacho RESULT result.

ENDCLASS.

CLASS lcl_Redespacho IMPLEMENTATION.
...
* ---------------------------------------------------------------------------
    ...
    reported-_redespacho = CORRESPONDING #( DEEP lt_reported-param  ).
...
* ---------------------------------------------------------------------------
    READ ENTITIES OF zc_tm_param_roadnet_new IN LOCAL MODE ENTITY _Redespacho
      ALL FIELDS
...
* ---------------------------------------------------------------------------
    result = VALUE #( BASE result FOR ls_item IN lt_item
                    ...
                    ( %tky                  = ls_item-%tky
                      ...
                      %action-importar_rede = COND #( WHEN ls_item-FreightOrder IS INITIAL
