CLASS zcltm_ce_monitor_gko_cte_comp DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcltm_ce_monitor_gko_cte_comp IMPLEMENTATION.
...
* ---------------------------------------------------------------------------
    TRY.
        ...
        lo_gko->fill_relatorio_cte_comp( EXPORTING iv_filter = io_request->get_filter( )->get_as_sql_string( )
                                         IMPORTING et_comp   = DATA(lt_result) ).
      CATCH cx_root.
    ENDTRY.
